...
void ranking(Graph *G){
	...
	int k=1 ,i; //Khoi tao rank ban dau (k) = 1
	while(S1.size>0){
		makeNullList(&S2);
		
		for(i=1; i<=S1.size; i++){
			u = elementAt(&S1,i); // u giu cac dinh trong list s1
			...
			for(v=1; v<=G->n; v++){
				if(G->l[u][v]!=0){ // dinh u khong muon lam lang gieng cua dinh khac vi da co rank
					...
					if(d[v]==0){
						pushBack(&S2, v);
					} // neu dinh v khong la lang gieng cua cac dinh khac => dinh goc dua vao list S2
...
int main(){
	Graph G;
	int i,m,n,u,v,j;
 	FILE* file = freopen("ThucHanh_buoi4_bai2.0.txt","r",stdin);
	scanf("%d%d",&n,&m);
	initGraph(&G,n);
	for(i=1; i<=m; i++){
		scanf("%d%d",&u,&v);
